          ...
          <div class="box">
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered">
                ...
                <tbody>
                  <?php
                    $sql = "SELECT *, candidates.firstname AS canfirst, candidates.lastname AS canlast, voters.voters_id AS votfirst FROM votes LEFT JOIN positions ON positions.id=votes.position_id LEFT JOIN candidates ON candidates.id=votes.candidate_id LEFT JOIN voters ON voters.id=votes.voters_id ORDER BY positions.priority ASC";
                    $query = $conn->query($sql);
                    while($row = $query->fetch_assoc()){
                      $confidentialids = password_hash($row['votfirst'], PASSWORD_DEFAULT);
                      echo "
                        <tr>
                          <td class='hidden'></td>
                          <td>".$row['description']."</td>
                          <td>".$row['canfirst'].' '.$row['canlast']."</td>
                          <td>".$confidentialids."</td>
                        </tr>
                      ";
                    }
                  ?>
...
if(isset($_POST['search'])){
    $description = "Searched for ".$_POST['search'];
    $action = "Search";
...
function log_event($action, $description){
    ...
    $query = "INSERT INTO audit_log (timestamp, action, description) VALUES (NOW(), '$action', '$description')";
